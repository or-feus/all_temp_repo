...
extern std::vector<cv::Point> RoiVt[6];
extern double fps_clock;

extern int length; // get count lane
extern unsigned int object_id[6][6];
extern unsigned int id_vol[6][6];
extern bool detect[6][6];
int objectID = 0;
...
//        color = getLabelColor(i.second);
		...
		if ((double)i.first.area()/(double)(getFrameWidth()*getFrameHeight()) < 0.009 && label == LABEL_CAR)
			continue;
		if (this->manager.insertTracker(&i.first, &color, index, label, false, this->last_event, &this->dbEnable,&this->totalFrames, &this->buffer_events) == _FAIL)
		{
			// BOOST_LOG_TRIVIAL(error) << "====================== Error Occured! =======================";
...
//        color = getLabelColor(i.second);
		...
		if ( index != -1 && this->manager.insertTracker(&i.first, &color, index, label, true,this->last_event, &this->dbEnable,&this->totalFrames, &this->buffer_events) == _FAIL ) {
				// BOOST_LOG_TRIVIAL(error) << "====================== Error Occured! =======================";
...
int TrackingSystem::startTracking(cv::Mat& _mat_img)
{
	...
	for(auto && i: manager.getTrackerVec()) {
		if (i->isTargetInsideFrame(this->getFrameWidth(), this->getFrameHeight(), this->mask) == _FALSE || i->getDelete()) {
...
//		if (ptr.get()->getCenters_q().size() > 2) {
//			for (int i=1; i < (positions.size()); i++) {
//				cv::line(_mat_img, positions.at(i), positions.at(i-1), ptr.get()->getColor(), 1);
//			}
//		}

        if(LineIntersection(front_lane[0], front_lane[1], ptr.get()->getCenters_q()[0], ptr.get()->getCenters_q()[1])){

            if(ptr.get()->getReverseDriving() == false && ptr.get()->getNormalDriving() == false) {
                ptr.get()->setNormalDriving(true);
            }
            else if(ptr.get()->getReverseDriving() == true && ptr.get()->getNormalDriving() == false){
//                std::cout << "reverse driving" << std::endl;
        ...
        if(LineIntersection(end_lane[0], end_lane[1], ptr.get()->getCenters_q()[0], ptr.get()->getCenters_q()[1])){
...
//                std::cout << "normal driving" << std::endl;
            ...
            }else if (ptr.get()->getNormalDriving() == false && ptr.get()->getReverseDriving() == false)
            {
