...
//        firstResults.push_back(std::make_pair(cv::Rect(cv::Point(x, y), cv::Size(label_size.width, label_size.height + baseLine)), obj.label));
...
//    }
    ...
    const std::string input_image_path = "/home/user/url.txt";
...
//        std::cout << "blob image" << std::endl;
        ...
        draw_lane(img, front_lane, SCALAR_WHITE);
        draw_lane(img, end_lane, SCALAR_WHITE);
        ...
        if(firstFrameWithDetections){
            tracking_system.setFrameWidth(img.cols);
            tracking_system.setFrameHeight(img.rows);
        ...
        int tracking_success = tracking_system.startTracking(img);
        if(tracking_success == _FAIL){
            break;
        }
        if(tracking_system.getTrackerManager().getTrackerVec().size() != 0){
            tracking_system.drawTrackingResult(img);
        }
