...
constexpr int _FAIL = -1;
constexpr int _SUCCESS = 1;
constexpr int _FALSE = 0;
constexpr int _TRUE = 1;
...
//    bool        driving_status;
    bool normal_driving;
    bool reverse_driving;
    bool on_timer;

    std::chrono::system_clock::time_point reverse_clocks;
    std::chrono::duration<double> clocks;
...
public:
    ...
    SingleTracker(int _target_id, cv::Rect _init_rect, cv::Scalar _color, int _label)
            : target_id(_target_id), confidence(0), is_tracking_started(false),
              c_q(boost::circular_buffer<cv::Point>(n_frames)), modvel(0), vel_x(0), vel_y(0), to_delete(false),
              no_update_counter(0), v_x_q(boost::circular_buffer<double>(n_frames_vel)),
              v_y_q(boost::circular_buffer<double>(n_frames_vel)), v_q(boost::circular_buffer<double>(n_frames_vel)),
              avg_pos(boost::circular_buffer<cv::Point>(n_frames_pos)),
              a_q(boost::circular_buffer<double>(n_frames_vel)), a_x_q(boost::circular_buffer<double>(n_frames_vel)),
              a_y_q(boost::circular_buffer<double>(n_frames_vel)), near_miss(false), collision(false), rect_width(1),
              b_areas(std::make_pair(0, nullptr)), normal_driving(false), reverse_driving(false), on_timer(false) {
        // Exception
    ...
    bool getNormalDriving() { return this->normal_driving; }

    bool getReverseDriving() { return this->reverse_driving; }

    bool getOnTimer() { return this->on_timer; }

    std::chrono::system_clock::time_point getReverseClocks() {return this->reverse_clocks;}

    std::chrono::duration<double> getClocks() { return this->clocks; }
    ...
    void setCenter(cv::Point _center) {
        this->center = _center;
        this->bottom = cv::Point(this->center.x, this->center.y + (this->rect.height / 2));
    }
    ...
    void setNormalDriving(bool _normal_driving) { this->normal_driving = _normal_driving; }

    void setReverseDriving(bool _reverse_driving) { this->reverse_driving = _reverse_driving; }

    void setOnTimer(bool _ontimer) { this->on_timer = _ontimer; }

    void setReverseClocks(std::chrono::system_clock::time_point _reverse_clocks) {this->reverse_clocks = _reverse_clocks;}

    void setClocks(std::chrono::duration<double> _clocks) {this->clocks = _clocks;}
    ...
    void saveLastVel(double _vel_x, double _vel_y, double _vel) {
        this->v_x_q.push_front(_vel_x);
        this->v_y_q.push_front(_vel_y);
        this->v_q.push_front(_vel);
    }

    void saveLastAcc(double _acc_x, double _acc_y, double _acc) {
        this->a_x_q.push_front(_acc_x);
        this->a_y_q.push_front(_acc_y);
        this->a_q.push_front(_acc);
    }
    ...
    void assignArea(std::vector <cv::Mat> *mask_sw, std::vector <cv::Mat> *mask_cw,
    ...
    // Do tracking
    int doSingleTracking(cv::Mat *_mat_img, std::vector <cv::Mat> *mask_sw, std::vector <cv::Mat> *mask_cw,
                         std::vector <std::pair<cv::Mat, int>> *mask_str, Pipe *buffer, int *totalFrames,
                         bool dbEnable);
...
public:
    /* Get Function */
    std::vector <std::shared_ptr<SingleTracker>> &
    getTrackerVec() { return this->tracker_vec; } // Return reference! not value!
    int getNextID() { return this->id_list; }

    /* Core Function */
    // Insert new SingleTracker shared pointer into the TrackerManager::tracker_vec
    int insertTracker(cv::Rect *_init_rect, cv::Scalar *_color, int _target_id, int _label, bool update,
...
public:
    ...
    explicit TrackingSystem(std::string *last_event) : last_event(last_event), mask(nullptr),
                                                       mask_sidewalks(nullptr), mask_streets(nullptr),
                                                       mask_crosswalks(nullptr), totalFrames(0), dbEnable(false) {
    };
    ...
    void setMask(cv::Mat *_mask, std::vector <cv::Mat> *_mask_crosswalks, std::vector <cv::Mat> *_mask_sidewalks,
                 std::vector <std::pair<cv::Mat, int>> *_mask_streets) {
        this->mask = _mask;
