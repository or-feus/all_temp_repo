...
void SingleTracker::calcVel() {
    const int full = 5; // const int full = 5;
    double delta_x = 0;
    double delta_y = 0;
    cv::Point2f avgvel;
    ...
    if (lim > 1) {
        ...
        this->setVel((cv::Point2f)
        this->getCenter() + avgvel);
...
void SingleTracker::calcAcc() {
    ...
    if (lim > 1) { // We need 2 speeds
        for (int i = 0; i < lim; i++) {
            delta_x = delta_x + ((this->v_x_q[i] + 1) * 1000.0 / (double) (this->avg_pos[i].y + 10) -
                                 (this->v_x_q[i + 1] + 1) * 1000.0 / (double) (this->avg_pos[i + 1].y + 10));
            delta_y = delta_y + ((this->v_y_q[i] + 1) * 1000.0 / (double) (this->avg_pos[i].y + 10) -
                                 (this->v_y_q[i + 1] + 1) * 1000.0 / (double) (this->avg_pos[i + 1].y + 10));
        }
        delta_x = delta_x / lim;
        delta_y = delta_y / lim;
        acc = cv::Point2f(delta_x, delta_y);
        this->setAcc((cv::Point2f)
        this->getCenter() + acc);
...
--------------------------------------------------------------------------------- */
void SingleTracker::assignArea(std::vector <cv::Mat> *mask_sw, std::vector <cv::Mat> *mask_cw,
                               std::vector <std::pair<cv::Mat, int>> *mask_str) {
    char areas = 0;
    cv::Point2f
    pos = (cv::Point2f)
    this->getBottom();
...
int TrackerManager::insertTracker(cv::Rect *_init_rect, cv::Scalar *_color, int _target_id, int _label, bool update,
                                  std::string *last_event, bool *dbEnable, int *totalFrames, Pipe *buffer) {
    // Exceptions
...
int TrackerManager::findTrackerByID(int _target_id) {
    auto target = find_if(tracker_vec.begin(), tracker_vec.end(),
                          [&_target_id](std::shared_ptr <SingleTracker> ptr) -> bool {
...
int TrackerManager::findTracker(cv::Rect rect, int label) {
    double max_overlap_thresh = 0.2;
    double dist_thresh =
            rect.height * rect.width >> 1; // Pixels^2 -> adjust properly (maybe a proportion of the img size?)
    ...
    for (auto &&s_tracker: this->getTrackerVec()) {
        ...
        if (max_per_area > max_overlap_thresh &&
            (s_tracker->getLabel() == label || s_tracker->getLabel() == LABEL_UNKNOWN)) {
...
----------------------------------------------------------------------------------- */
int
TrackerManager::deleteTracker(int _target_id, std::string *last_event, bool *dbEnable, int *totalFrames, Pipe *buffer) {
...
//        color = getLabelColor(i.second);
        ...
        if ((double) i.first.area() / (double) (getFrameWidth() * getFrameHeight()) < 0.009 && label == LABEL_CAR)
            continue;
        if (this->manager.insertTracker(&i.first, &color, index, label, false, this->last_event, &this->dbEnable,
                                        &this->totalFrames, &this->buffer_events) == _FAIL) {
            // BOOST_LOG_TRIVIAL(error) << "====================== Error Occured! =======================";
...
//        color = getLabelColor(i.second);
        ...
        if (index != -1 &&
            this->manager.insertTracker(&i.first, &color, index, label, true, this->last_event, &this->dbEnable,
                                        &this->totalFrames, &this->buffer_events) == _FAIL) {
            // BOOST_LOG_TRIVIAL(error) << "====================== Error Occured! =======================";
...
int TrackingSystem::startTracking(cv::Mat &_mat_img) {
    ...
    std::for_each(manager.getTrackerVec().begin(), manager.getTrackerVec().end(),
                  [&_mat_img](std::shared_ptr <SingleTracker> ptr) {
    ...
    // Multi thread
    std::for_each(manager.getTrackerVec().begin(), manager.getTrackerVec().end(),
                  [&thread_pool, &_mat_img, &mask_sw, &mask_cw, &mask_str, &buffer, &tFrames, &dbEn](
                          std::shared_ptr <SingleTracker> ptr) {
                      thread_pool.emplace_back(
                              [ptr, &_mat_img, &mask_sw, &mask_cw, &mask_str, &buffer, &tFrames, dbEn]() {
                                  ptr.get()->doSingleTracking(&_mat_img, mask_sw, mask_cw, mask_str, buffer, tFrames,
                                                              dbEn);
                              });
                  });
    ...
    for (auto &&i: manager.getTrackerVec()) {
        if (i->isTargetInsideFrame(this->getFrameWidth(), this->getFrameHeight(), this->mask) == _FALSE ||
            i->getDelete()) {
...
int TrackingSystem::drawTrackingResult(cv::Mat &_mat_img) {
    ...
    std::for_each(l_manager.getTrackerVec().begin(), l_manager.getTrackerVec().end(),
                  [&_mat_img](std::shared_ptr <SingleTracker> ptr) {
                      // Draw all rectangles
                      ...
                      if (LineIntersection(front_lane[0], front_lane[1], ptr.get()->getCenters_q()[0],
                                           ptr.get()->getCenters_q()[1])) {

                          if (zero_point_ext(ptr.get()->getCenters_q()[0], ptr.get()->getCenters_q()[1])) {
                              if (ptr.get()->getReverseDriving() == false && ptr.get()->getNormalDriving() == false) {
                                  ptr.get()->setNormalDriving(true);
                              } else if (ptr.get()->getReverseDriving() == true &&
                                         ptr.get()->getNormalDriving() == false) {
                                  ptr.get()->setColor(reverse_COLOR);

                              }
                          }
                      }

                      if (LineIntersection(end_lane[0], end_lane[1], ptr.get()->getCenters_q()[0],
                                           ptr.get()->getCenters_q()[1])) {
                          if (zero_point_ext(ptr.get()->getCenters_q()[0], ptr.get()->getCenters_q()[1])) {
                              if (ptr.get()->getNormalDriving() == true && ptr.get()->getReverseDriving() == false) {
                                  ptr.get()->setColor(normal_COLOR);
                              } else if (ptr.get()->getNormalDriving() == false &&
                                         ptr.get()->getReverseDriving() == false) {
                                  ptr.get()->setReverseDriving(true);
                              }
                          }
                      }

                      if (ptr.get()->getColor() == reverse_COLOR) {
                          is_reverse = true;
                          if(ptr.get()->getOnTimer() == false) {
                              ptr.get()->setReverseClocks(std::chrono::system_clock::now());
                              ptr.get()->setOnTimer(true);
                          }
                          ptr.get()->setClocks(std::chrono::system_clock::now() - ptr.get()->getReverseClocks());
//                          std::cout << "cnt_sec_tracking : " << static_cast<int>(ptr.get()->getClocks().count()) << std::endl;

                          if(static_cast<int>(ptr.get()->getClocks().count()) > 5) {
                              ptr.get()->setColor(default_COLOR);
                              ptr.get()->getReverseDriving() == false;
                              ptr.get()->getNormalDriving() == false;
                          }
                      }
